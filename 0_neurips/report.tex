...
We investigate and evaluate the effectiveness of various reinforcement learning (RL) algorithms in a custom high-dimensional tower defence environment, featuring a large discrete action space, stochastic dynamics, sparse rewards, and an automatic tower level-up mechanism. Algorithms such as Q-learning with Q-target networks, SARSA with Q-network approximation, Advantage Actor-Critic (A2C), Proximal Policy Optimization (PPO), Deep Q-Networks (DQN), and Quantile Regression DQN (QR-DQN) were evaluated in their ability to learn an optimal policy. Our results show that tabular-inspired methods such as Q-learning and SARSA struggle due to the environment's non-stationary dynamics, yielding high variance and poor convergence. Policy gradient-based algorithms such as A2C and PPO achieved better results, but exhibited instability and sensitivity to hyperparameters. QR-DQN, a distributional approach meant to stabilize traditional DQN learning, showed little performance improvement on traditional DQN - suggesting that modelling return uncertainty is provides limited benefit in this environment. Overall, DQN was the best performing algorithm, combining experience replay and target networks to achieve high returns.
...
\begin{itemize}
  ...
  \item Enemy Reaches Base = -15
  \item Enemy Damaged = +1
  \item Tower Level Up = +5
  \item Wave Cleared = +20
  \item All Waves Cleared = +200
  \item Base Destroyed = -50
  \item Invalid or No Action = -1
\end{itemize}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.5\linewidth]{figures/qlearning_10runs_10000episodes.png}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.5\linewidth]{figures/sarsa\_10runs\_10000episodes.png}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/a2c\_learn\_1runs\_10000episodes.png}
...
To attempt to address the instability observed in standard DQN, we implemented Quantile Regression DQN (QR-DQN), which is a distributional RL algorithm \cite{dabney2018qrdqn}. Rather than learning a single expected Q-value, QR-DQN models the full distribution of returns using a set of quantiles. This may provide better information about uncertainty in value estimates, which can improve learning stability, especially in a more stochastic environment like this. This algorithm was implemented using the SB3 library with 50 quantiles. 
